...
namespace GilqEngine
{
    ...
    void Mesh::drawInstanced(Shader *shader, u32 numberOfInstances)
    {
        TRACE();
        _vertexData.bind();
        _material.setShaderUniforms(shader);
    ...
    void Mesh::updateBufferFloat2(u32 layoutIndex, const void *data, u32 numberOfUpdates)
    {
        TRACE();
        _vertexData.updateBufferFloat2(layoutIndex, data, numberOfUpdates * sizeof(float) * 2);
    }

    void Mesh::updateBufferFloat3(u32 layoutIndex, const void *data, u32 numberOfUpdates)
    {
        TRACE();
        _vertexData.updateBufferFloat3(layoutIndex, data, numberOfUpdates * sizeof(float) * 3);
    }

    void Mesh::updateBufferFloat4(u32 layoutIndex, const void *data, u32 numberOfUpdates)
    {
        TRACE();
        _vertexData.updateBufferFloat4(layoutIndex, data, numberOfUpdates * sizeof(float) * 4);
    }

    void Mesh::updateBufferMat4(u32 layoutIndex, const void *data, u32 numberOfUpdates)
    {
        TRACE();
